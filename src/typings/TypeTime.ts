...
class ExtensionTime {
  ...
  static formatDate(date: Date, strFormat: string, constant: IConstant) {
    ...
    strFormat = strFormat.replace('MMMM', nameLongMonths[date.getUTCMonth()]);
    strFormat = strFormat.replace('MM', nameShortMonths[date.getUTCMonth()]);
  ...
  static formatTimeByDate(date: Date, locale: Locale) {
    const {typeTime} = CONSTANTS_UNIT[locale];
    let h = date.getHours();
    const m = date.getMinutes();
    if (typeTime === TypeTime.clock24) {
      return `${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}`;
    } else {
      const time = h > 11 ? TimeOfDay.PM : TimeOfDay.AM;
      if (h !== 12) {
        h %= 12;
      }
