...
const EventScreen = ({navigation, route}: EventScreenProps) => {
  ...
  const renderItem = (info) => {
    ...
    if (item.type === FilterEvents.birthday) {
      return (
        <EventBirthday key={event._id} user={item.payload as IUser} date={item.date} conStyle={styles.eventBlock} />
      );
    } else if (item.type === FilterEvents.mine) {
      return (
        <MineEvent key={event._id} onPress={handleSelect} isOpen={isOpen} event={event} conStyle={styles.eventBlock} />
      );
