...
const InvitedEvent = ({conStyle, defaultStatus, event, isOpen, onPress}: IInvitedEventProps) => {
  ...
  const [statusEvent, setStatusEvent] = useState<StatusAnswerEvent | undefined>(defaultStatus);
  ...
  const refPrevStatus = useRef<StatusAnswerEvent | undefined>(statusEvent);
  ...
  const handleChangeStatus: any = useCallback(
    debounce(async (status: StatusAnswerEvent) => {
      ...
      if (res.success && refPrevStatus.current && compareStatus(refPrevStatus.current, status)) {
  ...
  const handleNavigateToChat = async (isSecret: boolean) => {
    ...
    const idChat = isSecret ? event.chats[1] : event.chats[0];

    if (!findChat && idChat) {
      const chat = await chatService.getChatById(getIdObj(idChat));
      if (chat.success) {
        findChat = chat.data!;
        dispatch(actionsEvent.addChat(findChat));
      }
    }
    if (findChat) {
