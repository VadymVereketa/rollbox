...
creator.addAction<IEvent>('updateEvent', (state, action) => {
  const {events} = state;
  const findIndex = events.findIndex((w) => w._id === action.payload._id);

  if (findIndex === -1) {
  } else {
    delete action.payload.guests;
    delete action.payload.owner;
    const event: IEvent = {...state.events[findIndex], ...action.payload};
    state.events = state.events.filter((_, i) => i !== findIndex);
...
creator.addAction<IMessageUpdate>('updateChat', (state, action) => {
  ...
  state.chats[findIndex] = {...chat, lastMessage, updatedAt: date};
